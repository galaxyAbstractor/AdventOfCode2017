const input = ("737\t1866\t1565\t1452\t1908\t1874\t232\t1928\t201\t241\t922\t281\t1651\t1740\t1012\t1001\n" +
		...
		"55\t258\t363\t116\t319\t49\t212\t44\t303\t349\t327\t330\t316\t297\t313\t67").split("\n").map((row) => {
	return row.split("\t")
});

function smallest(acc, current) {
	return Number(current) < Number(acc) ? current : acc;
}

function largest(acc, current) {
	return Number(current) > Number(acc) ? current : acc;
}

const sum = input.reduce((acc, current) => {
	let small = current.reduce(smallest, Number.MAX_VALUE);
	let large = current.reduce(largest, Number.MIN_VALUE);

	return acc + (Number(large) - Number(small));
}, 0);
