...
const sum = input.reduce((acc, current) => {
	...
	for(let i = 0; i < current.length; i++) {
		let divisible = current.find((curr, index) => (index !== i) &&
																									(curr % current[i] === 0 || current[i] % curr === 0));

		if (divisible) {
			divisibleNumbers.push(Math.max(divisible, current[i]));
			divisibleNumbers.push(Math.min(divisible, current[i]));
			break;
		}
	}

	return acc + (divisibleNumbers.shift() / divisibleNumbers.shift());
}, 0);
